...
/******** OWNER NUMBER**********/
const ownerNumber = ["553186283462@s.whatsapp.net"]   //ganti nomor lu! 
const pacarNumber = ["553186283462@s.whatsapp.net"]  //ganti nomor lu!  
/************************************/
	...
	client.on('message-new', async (mek) => {
        ...
        if (levelRole <= 2) {
           ...
           var prema = 'Free'
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("TOME CUIDADO COM A FALA DE DONG!! 😠")
		...
		setTimeout( () => {
			...
			switch(command) {
                ...
                if (args.length < 1) return reply('1 ativar, 0 desativar')
                if (args[0] === '1') {
                if (isBadWord) return reply('*O recurso BadWord já estava ativo antes*')
                 	   badword.push(from)
				...
				case 'addvn':  // Fix Bug By XYTODBot & Yogi*
					...
					if (!isQuotedAudio) return reply('Responder vnnya blokk!')
					svst = body.slice(7)
				...
				case 'addimage':  // Fix Bug By XYTODBot & Yogi*
					...
					if (!isQuotedImage) return reply('Responder a imagem quadra!')
					svst = body.slice(10)
					if (!svst) return reply('Qual é o nome da imagem su?')
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await client.downloadMediaMessage(boij)
					imagenye.push(`${svst}`)
				...
				case 'addvideo':  // Fix Bug By XYTODBot & Yogi*
					...
					if (!isQuotedVideo) return reply('Responder o vídeo quadra!')
					svst = body.slice(10)
					if (!svst) return reply('Qual é o nome do vídeo ku?')
					boij = JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
					delb = await client.downloadMediaMessage(boij)
					videonye.push(`${svst}`)
		...
		case 'mútuo':// Fix Case By Yogi/Hans⛔
		...
		case 'lb':
				...
				let leaderboardlvl = '-----[ *ENTRE OS MELHORES NÍVEL* ]----\n\n'
		...
		case 'kalkulator':
				...
				mtk = body.slice(12)
				if (typeof Math_js.evaluate(mtk) !== "número") {
		...
		case 'giftlimit': 
				// Fix Case By Yogi/Hans⛔
                 if (!isRegistered) return reply( ind.noregis())
				if (!isAdmin) return reply('*Apenas Admin & Proprietário Dudu!*')
